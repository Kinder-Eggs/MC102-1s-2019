...
def resolve(resposta):
	...
	for i in range(len(resposta)):  # iterar linha por linha
		try:  # tenta encontrar 0 na linha
			j = resposta[i].index(0)
			while True:  # aumenta o valor de 1 em 1 e testa se eh valido, se for, chama a funcao novamente
				if resposta[i][j] == 9:  # para apos chegar no valor maximo
					resposta[i][j] = 0
					return False
				resposta[i][j] += 1
				if checkvalido(resposta, i, j) == True:
					if resolve(resposta) == False:
						continue
					return True  # quando encontrado a resposta retorna True
		except:  # caso nao haja zero passa para a proxima linha
			continue
